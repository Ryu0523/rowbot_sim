...
class USVControlEnv:
    ...
    def __init__(self, db=None, db_path="hydro_wigley_10m.npz",
                 hs=3.25, tp=9.7, theta0=np.pi, n_freq=24, n_dir=1,
                 dt=0.05, dt_ctrl=0.5, t_end=200.0, u_ref=4.5,
                 t_preview=8.0, n_preview=6, n_station=3,
        ...
        self.db = db if db is not None else bem.load(db_path)
    ...
    def _build(self, seed):
        ...
        self._plant = NonlinearVessel(
            self.db, sea, L=self.db.L,
            B=float(self.db.attrs.get("B", 2.5)),
            T=float(self.db.attrs.get("T", 0.8)), dt=self.dt)
        self.sea = sea
    ...
    def _observe(self):
        ...
        base = np.array([u / self.u_ref, v, z, w_, pitch, s[10],
                         _wrap(psi - self.heading_ref), r,
                         p.last_bow_acc / G], float)
...
def main():
    print("direct-control environment: action = (thrust, rudder)\n")
    env = USVControlEnv()
